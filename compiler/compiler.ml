 ...
 *)
 ...
 module Reader : READER = struct
   ...
   and nt_vector str =
     ...
     let nt3 = pack nt3 (fun s ->ScmVector s) in
     let nt4 = pack nt_skip_star (fun _ ->ScmVector []) in
     let nt3 = disj nt3 nt4 in
   ...
   and pair_rec lst =
     match lst with
     | [] -> ScmNil
     | a::b -> ScmPair(a,pair_rec b)
   and nt_proper_list str = 
     let nt1 = char '(' in
     let nt2 = char ')' in
     let nt3 = star nt_sexpr in
     let nt1 = caten nt1 (caten nt3 nt2) in (*fold_right*)
     let nt1 = pack nt1 (fun (_,(lst,_)) -> pair_rec lst) in
     nt1 str
   and nt_improper_list str =
     ...
     let nt1 = pack nt1 (fun (_, (lst_exp, (_, (imp_exp ,_))))-> List.fold_right (fun a b -> ScmPair(a,b)) lst_exp imp_exp) in
     nt1 str
    and nt_list str =
 ...
 let aight str = (Reader.nt_sexpr str 0);;
 ...
 module Tag_Parser : TAG_PARSER = struct
   ...
   let rec macro_expand_qq = function
     ...
     | ScmPair (car, cdr) ->
       ...
       ScmPair(
         ScmSymbol "cons", ScmPair(expanded_car, ScmPair(expanded_cdr, ScmNil)))
     | ScmVector sexprs ->
   ...
   let rec macro_expand_cond_ribs ribs = 
     ...
     match ribs with
     | ScmNil -> ScmVoid
     | ScmPair(ScmPair(ScmSymbol "else", sexpr),ribs) ->
       ScmPair(ScmSymbol "begin", sexpr)
     | ScmPair(ScmPair(exp,
                 ScmPair(ScmSymbol "=>",
                           ScmPair(lambd, ScmNil))), ribs) ->
       let rest = macro_expand_cond_ribs ribs in
         ScmPair
           (ScmSymbol "let",
             ScmPair
               (ScmPair
                 (ScmPair (ScmSymbol "value", ScmPair (exp, ScmNil)),
                   ScmPair
                     (ScmPair
                       (ScmSymbol "f",
                         ScmPair
                           (ScmPair
                             (ScmSymbol "lambda",
                               ScmPair (ScmNil, ScmPair (lambd, ScmNil))),
                                 ScmNil)),
                                   ScmPair
                                     (ScmPair
                                       (ScmSymbol "rest",
                       ...
                       (ScmPair
                         (ScmPair (ScmSymbol "f", ScmNil),
                           ScmPair (ScmSymbol "value", ScmNil)),
                               ...
                               ScmNil)))
     | ScmPair (ScmPair (ifcond, exprs), ribs) ->
         let rest = macro_expand_cond_ribs ribs in
         ScmPair (ScmSymbol "if",
                   ScmPair(ifcond,
                         ScmPair(
   ...
   (*from here --- help funcs for letrec tagparse*)
   let rec extract_vars ribs =
     match ribs with
     | ScmNil -> ScmNil
     | ScmPair(ScmPair(ScmSymbol(v),ScmPair(_,ScmNil)), rest_ribs) -> ScmPair(ScmSymbol(v), extract_vars rest_ribs)
     | _ -> raise (X_syntax "Error in extracting vars for letrec expansion");;
 
   let rec give_vars_whateves vars =
     match vars with
     | ScmNil -> ScmNil
     | ScmPair(ScmSymbol(curr), rest) -> ScmPair(ScmPair(ScmSymbol(curr), ScmPair(ScmPair(ScmSymbol("quote"), ScmPair(ScmSymbol("whatever"), ScmNil)),ScmNil)), give_vars_whateves rest)
     | _ -> raise (X_syntax "Error in giving vars 'whatever value in letrec expansion");;
 
   let rec make_sets1 ribs = 
     match ribs with
     | ScmNil -> ScmNil
     | ScmPair(ScmPair(ScmSymbol(v),ScmPair(sexpr,ScmNil)),rest) -> ScmPair(ScmPair(ScmSymbol "set!" , ScmPair(ScmSymbol v, ScmPair(sexpr, ScmNil))),make_sets1 rest)
     | _ -> raise (X_syntax "Error in making sets in letrec expansion");;
 
   let rec connect_sets_let sets let_sexprs =
     match sets, let_sexprs with
     | ScmNil, ScmNil -> ScmNil
     | ScmPair(set, rest), rest_exps -> ScmPair(set,connect_sets_let rest rest_exps)
     | ScmNil, ScmPair(memb, rest) -> ScmPair(memb, connect_sets_let ScmNil rest)
     | _ -> raise (X_syntax "Error in connect_sets_let in letrec expansion");;
   (*to here --- help funcs for letrec tagparse*)
   ...
   let rec tag_parse sexpr =
     ...
     | ScmSymbol var ->
        ...
        then raise (X_syntax (Printf.sprintf "Variable cannot be a reserved word ==> %s" var))
     ...
     | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       ...
       let vars_whateves = give_vars_whateves vars in
       let set_skele = make_sets1 ribs in
       let connected_sets_let_exps = connect_sets_let set_skele exprs in
       let expanded = ScmPair (ScmSymbol "let", ScmPair(vars_whateves, connected_sets_let_exps)) in
       tag_parse expanded
 ...
 module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
   ...
   let annotate_lexical_address pe =
     (*implemented for hw 2*)
     let rec tag pe params env =
       match pe with
       | ScmConst sexp -> ScmConst' sexp
       | ScmVarGet (Var var) -> ScmVarGet' (tag_lexical_address_for_var var params env)
       | ScmIf(test, dit, dif) -> ScmIf' ((tag test params env), (tag dit params env), (tag dif params env))
       | ScmSeq (exprs) -> ScmSeq' (List.map (fun expr -> tag expr params env) exprs)
       | ScmOr (exprs) -> ScmOr' (List.map (fun expr -> tag expr params env) exprs)
       | ScmVarSet (Var var, expr) -> ScmVarSet' ((tag_lexical_address_for_var var params env), (tag expr params env))
       | ScmVarDef (Var var, expr) -> ScmVarDef' ((tag_lexical_address_for_var var params env), (tag expr params env))
       | ScmLambda (paramses, Simple, expr) -> ScmLambda' (paramses, Simple , (tag expr paramses ([params] @ env)))
       | ScmLambda (paramses, Opt opt , expr) -> ScmLambda' (paramses, Opt opt , (tag expr (paramses @ [opt]) ([params] @ env)))
       | ScmApplic (proc, args) -> ScmApplic' ((tag proc params env), (List.map (fun expr -> tag expr params env) args), Non_Tail_Call)
     in
     tag pe [] [];;

   let annotate_tail_calls pe = 
     (*implemented for hw 2*)
     let rec ann in_tp = function 
     | (ScmConst' _) as expr' -> expr'
     | (ScmVarGet' _) as expr' -> expr'
     | ScmIf' (test, dit, dif) -> ScmIf' ((ann false test), (ann in_tp dit), (ann in_tp dif))
     | ScmSeq' (exprs) -> ScmSeq' (ann_seq in_tp exprs)
     | ScmOr' (exprs) -> ScmOr' (ann_seq in_tp exprs)
     | ScmVarSet' (var', exp') -> ScmVarSet' (var', (ann false exp'))
     | ScmVarDef' (var', exp') -> ScmVarDef' (var', (ann false exp'))
     | (ScmBox' _) as expr' -> expr'
     | (ScmBoxGet' _) as expr' -> expr'
     | ScmBoxSet' (var', expr') -> ScmBoxSet' (var', (ann false expr'))
     | ScmLambda' (params, Simple, expr) -> ScmLambda' (params, Simple, (ann true expr))
     | ScmLambda' (params, Opt opt, expr) -> ScmLambda' (params,Opt opt,(ann true expr))
     | ScmApplic' (proc, args, app_kind) -> 
       if in_tp
         then ScmApplic' (ann false proc,
                          List.map (fun arg -> ann false arg) args,
                          Tail_Call)
         else ScmApplic' (ann false proc,
                          List.map (fun arg -> ann false arg) args,
                          Non_Tail_Call)
 
     and ann_seq in_tp exprs =
       match exprs with
       | ex :: [] -> [ann in_tp ex]
       | ex :: exprsss -> (ann false ex) :: (ann_seq in_tp exprsss)
       | [] -> raise (X_syntax "hmmmmmm.... in line 1092")
     in
     ann false pe;;
 ...
 let ann str = Semantic_Analysis.annotate_lexical_address (parse str);;
 ...
 module Code_Generation : CODE_GENERATION = struct
   ...
   let global_bindings_table =
     [ (* 1-10 *)
       ...
       ("eq?", "L_code_ptr_is_eq");
       ("__integer-to-fraction", "L_code_ptr_integer_to_fraction");(*changed to _ from __*)
   ...
   let code_gen exprs' =
     ...
     let rec run params env = function
       ...
       | ScmIf' (test, dit, dif) ->
         (*implemented for final project*)
         let _end = make_if_end () in
         let _else = make_if_else () in
         (run params env test) ^ "\tcmp rax, sob_boolean_false\n"
         ^ (Printf.sprintf "\tje %s\n" _else) ^ (run params env dit)
         ^ (Printf.sprintf "\tjmp %s\n" _end) ^ (Printf.sprintf "\t%s:\n" _else)
         ^ (run params env dif) ^ (Printf.sprintf "\t%s:\n" _end)
       | ScmSeq' exprs' ->
          String.concat "\n"
       ...
       | ScmVarSet' (Var' (v, Free), expr') ->
         (*implemented for final project*)
         let val_free = search_free_var_table v free_vars in
         (run params env expr') ^ (Printf.sprintf "\tmov qword [%s], rax\n" val_free)
         ^ "\tmov rax, sob_void\n"
       ...
       | ScmVarSet' (Var' (v, Bound (major, minor)), expr') ->
         (*implemented for final project*)
         (run params env expr') ^ "\t mov rbx, qword [rbp + 8 * 2]\n"
       ...
       | ScmLambda' (params', Opt opt, body) ->
         ...
         let label_loop_env = make_lambda_opt_loop_env ()
           ...
           (*more labels for lambda opt...*)
           and opt_lambda_more = make_lambda_opt_arity_more()
           and opt_loop = make_lambda_opt_arity_exact()
           and shift_loop_1 = make_lambda_opt_loop()
           and shift_loop_1_end = make_lambda_opt_loop_exit()
           and stack_adjusted = make_lambda_opt_stack_ok()
           and opt_more_loop = make_lambda_opt_loop()
           and opt_more_loop_end = make_lambda_opt_loop_exit()
           and opt_shrink_stack_loop = make_lambda_opt_loop()
           and opt_shrink_stack_loop_end = make_lambda_opt_end()
           and args_len = (List.length params') + 1
         in
           "\tmov rdi, (1 + 8 + 8)\t; sob closure\n"
           ^ "\tcall malloc\n"
           ^ "\tpush rax\n"
           ...
           (*added for lambda opt*)
           ^"\tpush rbp\n"
           ^(Printf.sprintf "mov rbx, %d\n" (args_len))
           ^"\tmov rcx, qword [rsp + 8*3]  ; move the given argument count in rcx\n"
           ^"\tadd rcx, 1     ; is exact if no opt\n"
           ^"\tcmp rcx, rbx   ; compare the num of arguments and act accordinly\n"
           ^(Printf.sprintf "\tje %s\n" opt_loop)
           ^"\tcmp rcx, rbx\n"
           ^(Printf.sprintf "\tjg %s\n" opt_lambda_more)
           ...
           ^(Printf.sprintf "\t%s:\n" shift_loop_1)
           ^"\tcmp rcx, rdx ; if we went over all elements\n"
           ^(Printf.sprintf "\tje %s\n" shift_loop_1_end)
           ^"\tmov rbx, qword [rsp + 8*rdx]     ; save the element of index rdx in frame\n"
           ^"\tmov qword [rsp + 8*rdx - 8], rbx ; move the element one position down\n"
           ^"\tinc rdx\n"
           ^(Printf.sprintf "\tjmp %s\n" shift_loop_1)
           ^(Printf.sprintf "\t%s:\n" shift_loop_1_end)
           ^"\tmov qword [rsp + 8*rdx - 8], sob_nil ; move empty list to the prev last index in frame\n"
           ^"\tsub rsp, 8    ; update the stack pointer after shift\n"
           ^(Printf.sprintf "\tmov qword [rsp + 8*3], %d; update count\n" (args_len))
           ^(Printf.sprintf "\tjmp %s\n" stack_adjusted)          
           ^"\t; CASE MORE ARGS\n"
           ^(Printf.sprintf "\t%s:\n" opt_lambda_more)
           ^(Printf.sprintf "\tmov rax, %n\n" (args_len))
           ^"\tmov rdi, rax\n"
           ^"\tmov rsi, rax\n"
           ^"\tsub rsi, 1\n"
           ^"\tmov rcx, qword [rsp + 8*3]  \n"
           ^"\tsub rcx, 1 \n"
           ^"\tmov rdx, sob_nil ; rdx contains the prev pair\n"
           ^(Printf.sprintf "\t%s:\n" opt_more_loop)
           ^"\tcmp rcx, rsi\n"
           ^(Printf.sprintf "\tjl %s\n" opt_more_loop_end)
           ^"\tmov rbx, qword [rsp + 8*(4+rcx)] \n"
           ^"\tpush rdi\n"
           ^"\tmov rdi, 17      ;pair (1 + 8 + 8 = 17)\n"
           ^"\tcall malloc\n"
           ^"\tpop rdi\n"
           ^"\tmov byte [rax], T_pair \n"
           ...
           ^"\tadd rcx, 3\n"  
           ^"\tadd rsi, 3\n" 
           ^(Printf.sprintf "\t%s:\n" opt_shrink_stack_loop)
           ^"\tcmp rsi, 0\n"     
           ^(Printf.sprintf "\tjl %s\n" opt_shrink_stack_loop_end)
           ^"\tmov rbx, qword[rsp + 8*rsi] ; get element at index rsi\n" 
           ^"\tmov qword[rsp + 8*rcx], rbx         ; put the element in index rcx\n" 
           ^"\tdec rsi\n" 
           ^"\tdec rcx\n" 
           ^(Printf.sprintf "\tjmp %s\n" opt_shrink_stack_loop)
           ^(Printf.sprintf "\t%s:\n" opt_shrink_stack_loop_end)
           ^"\tsub rdx, rdi\n"
           ^"\tlea rsp,[rsp + 8*rdx] ; fix the stack pointer\n" 
           ^"\tmov qword [rsp + 8*3], rdi ; update count\n" 
           ^(Printf.sprintf "\t%s:\n" stack_adjusted)
           ^"\tpop rbp\n"
           (*end of added for lambda opt*)
           ^ "\tenter 0, 0\n"
           ^ (run (args_len) (env + 1) body)
           ^ "\tleave\n"
           ^ (Printf.sprintf "\tret 8 * (2 + %d)\n" (args_len))
           ^ (Printf.sprintf "%s:\t; new closure is in rax\n" label_end)
       | ScmApplic' (proc, args, Non_Tail_Call) -> 
         (*implemented for final project*)
         let argsLen = string_of_int (List.length args) in
         let revlist = List.rev args in 
         let args_appliction = String.concat ""(List.map (fun arg->(run params env arg)^"\tpush rax\n") revlist) in
         args_appliction 
